    ...
    >
      ...
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-gray-950">
        <img
          ...
          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 brightness-[0.85] group-hover:brightness-90"
        />

        {/* Shading Radial Overlay */}
        ...
        <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-md px-1.5 py-1.5 rounded-lg border border-stone-150 flex items-center justify-center shadow-md">
          <div className="w-4 h-4 border-2 border-green-600 p-[2px] flex items-center justify-center bg-transparent rounded-sm" title="Pure Veg">
            <div className="w-2 h-2 rounded-full bg-green-600" />
          </div>
        </div>

        {/* Bestseller or Chef Special Ribbon */}
        {item.isChefSpecial ? (
          <div className="absolute top-3 right-3 bg-gradient-to-r from-red-600 to-amber-600 text-white text-[10px] sm:text-xs font-semibold tracking-widest uppercase px-3 py-1 rounded-full shadow-md flex items-center gap-1">
            <Sparkles className="w-3 h-3 text-amber-200 animate-pulse" />
            Chef's Special
          </div>
        ) : item.isBestseller ? (
          <div className="absolute top-3 right-3 bg-gradient-to-r from-[#d4af37] to-[#aa7c11] text-black text-[10px] sm:text-xs font-bold tracking-widest uppercase px-3 py-1 rounded-full shadow-md flex items-center gap-1">
            <ThumbsUp className="w-3 h-3 text-black" />
            Bestseller
          </div>
        ) : null}

        {/* Rating and Reviews Counter badge bottom right */}
        <div className="absolute bottom-3 right-3 bg-white/95 backdrop-blur-md px-2 py-1 rounded-lg border border-stone-200/50 flex items-center gap-1 shadow-sm text-[11px] font-medium text-stone-700">
          <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
          <span>{item.rating.toFixed(1)}</span>
          <span className="text-gray-500 font-light">({item.ratingCount})</span>
        </div>
      </div>
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-grow justify-between">
        <div>
          ...
          <div className="flex items-start justify-between gap-2 mb-2">
            <h3 className="text-base sm:text-lg font-serif font-semibold text-stone-850 tracking-wide group-hover:text-[#aa7c11] transition-colors duration-200 line-clamp-1">
              {item.name}
            </h3>
            {renderSpiciness()}
          </div>
