...
export default function MobileView({
  ...
  const [activeTab, setActiveTab] = useState<"home" | "menu" | "search" | "cart" | "contact">("home");
  ...
  const [checkoutErrorMobile, setCheckoutErrorMobile] = useState<string | null>(null);
  ...
  const restaurantWhatsAppNumber = storedSettings.whatsappNumber ? storedSettings.whatsappNumber.replace(/[^0-9]/g, "") : "919630013483"; 
  ...
  const reviewsData = [
    {
      id: "rev-m1",
      name: "Aaryan Rajput",
      comment: "The absolute gold standard for pure vegetarian North Indian and South Indian dining. Their Paneer Butter Masala and Masala Dosa are simply legendary!",
      rating: 5,
    ...
    },
    {
      id: "rev-m2",
      name: "Rohan Mehra",
      comment: "Incredibly fast delivery and premium packaging. The Masala Papad was crispy and Veg Kurkure Momos were still piping hot!",
      rating: 5,
    ...
    },
    {
      id: "rev-m3",
      name: "Neha Sharma",
      comment: "Exceptional South Indian heritage flavors. The Mysore Dosa had the perfect level of red-chili garlic spice. Highly recommended!",
      rating: 5,
  ...
  const subtotal = useMemo(() => cart.reduce((sum, item) => sum + item.menuItem.price * item.quantity, 0), [cart]);
    ...
    return menuList.filter((item) => {
      ...
      if (selectedMobileCategory !== "all") {
        if (selectedMobileCategory === "south-indian") {
          if (!["idli", "uttapam", "dosa"].includes(item.category)) return false;
        } else if (selectedMobileCategory === "drinks") {
          if (!["milkshakes", "mocktails", "tea-coffee", "refreshers"].includes(item.category)) return false;
        } else {
  ...
  const handleCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    if (otpCodeMobile !== generatedOtpMobile) {
      setOtpErrorMobile("Incorrect One-Time Passcode. Please check the simulator bypass code.");
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setCheckoutErrorMobile(err.message || "Failed to finalize order checkout structure.");
    } finally {
  ...
  const handleQuickWhatsAppQuery = (e: React.FormEvent) => {
    ...
    window.open(`https://wa.me/${restaurantWhatsAppNumber}?text=${encoded}`, "_blank");
    setShowQuickOrder(false);
  };

  return (
    <div className="bg-[#FAF9F5] text-stone-850 min-h-screen flex flex-col font-sans select-none antialiased relative pb-24" id="mobile-viewport">
      ...
      {animateContainer(isLoading, activeTab, {
        home: (
          <div className="space-y-8 animate-fade-in pb-12">
            ...
            <div className="relative overflow-hidden rounded-3xl mx-4 mt-4 h-[340px] bg-cover bg-center shadow-[0_12px_40px_rgba(40,30,10,0.12)]" style={{
              backgroundImage: "linear-gradient(to bottom, rgba(0,0,0,0.1), rgba(0,0,0,0.9)), url('https://images.unsplash.com/photo-1544025162-d76694265947?auto=format&fit=crop&q=80&w=800')"
            }}>
              ...
              <div className="absolute top-4 right-4 bg-black/85 backdrop-blur-md p-2 rounded-2xl border border-gray-800/80 shadow-md">
                <span className="text-sm font-serif font-extrabold text-[#d4af37]">SR</span>
              </div>

              {/* Hero Content Bottom */}
              <div className="absolute bottom-6 left-6 right-6 space-y-3">
                <span className="text-[10px] font-mono tracking-[0.25em] text-[#d4af37] uppercase font-bold">ESTD 2004 • NEW DELHI</span>
            ...
            <div className="mx-4 bg-white p-4 rounded-2xl border border-stone-200 shadow-sm flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-stone-900 uppercase tracking-wider">Free Delivery Coupon</h4>
                  <p className="text-[10px] text-stone-500 mt-0.5">Auto-applies above ₹400 orders</p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="px-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-serif font-bold text-stone-900 uppercase tracking-wide">Chef&apos;s Specials</h2>
                </div>
                <span className="text-[10px] font-mono text-stone-400 uppercase">Swipe Left</span>
              </div>

              {/* Horizontal Scroll wrapper */}
              <div className="overflow-x-auto pb-4 px-4 flex gap-4 scrollbar-none snap-x snap-mandatory">
                {chefSpecialsList.map((item) => {
                  const qty = cart.find((ci) => ci.menuItem.id === item.id)?.quantity || 0;
                  return (
                    <div 
                      key={item.id} 
                    ...
                    >
                      ...
                      <div className="p-4 flex-grow flex flex-col justify-between space-y-3">
                        <div className="space-y-1">
                          <h3 className="text-sm font-serif font-bold text-stone-900 line-clamp-1">{item.name}</h3>
                          <p className="text-[11px] text-stone-550 line-clamp-2 leading-relaxed font-light">{item.description}</p>
                        </div>
                        
                        <div className="flex items-center justify-between pt-2 border-t border-stone-100">
                          <span className="text-sm font-mono text-[#aa7c11] font-semibold">₹{item.price}</span>
                          ...
                          {qty > 0 ? (
                            <div className="flex items-center gap-2 bg-[#d4af37] rounded-xl p-0.5 border border-[#d4af37]">
                              ...
                              <span className="text-black font-mono font-bold text-xs w-4 text-center">{qty}</span>
                              <button
            ...
            <div className="px-4 space-y-4">
              <h2 className="text-lg font-serif font-bold text-stone-900 uppercase tracking-wide">Popular Flavors</h2>
              <div className="grid grid-cols-2 gap-3">
                <div onClick={() => { setSelectedMobileCategory("south-indian"); setActiveTab("menu"); }} className="relative h-24 rounded-2xl overflow-hidden cursor-pointer group shadow-[0_8px_25px_rgba(40,30,10,0.05)]">
                  <img src="https://images.unsplash.com/photo-1668236543090-82eba5ee5976?auto=format&fit=crop&q=80&w=300" className="w-full h-full object-cover brightness-[0.7] group-hover:scale-105 transition-all duration-300" alt="South Indian" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                  <span className="absolute bottom-3 left-3 text-xs font-serif font-bold text-white">🥞 South Indian</span>
                </div>
                <div onClick={() => { setSelectedMobileCategory("chinese"); setActiveTab("menu"); }} className="relative h-24 rounded-2xl overflow-hidden cursor-pointer group shadow-[0_8px_25px_rgba(40,30,10,0.05)]">
                  <img src="https://images.unsplash.com/photo-1585032226651-759b368d7246?auto=format&fit=crop&q=80&w=300" className="w-full h-full object-cover brightness-[0.7] group-hover:scale-105 transition-all duration-300" alt="Chinese" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                  <span className="absolute bottom-3 left-3 text-xs font-serif font-bold text-white">🥢 Chinese Wok</span>
                </div>
                <div onClick={() => { setSelectedMobileCategory("main-course"); setActiveTab("menu"); }} className="relative h-24 rounded-2xl overflow-hidden cursor-pointer group shadow-[0_8px_25px_rgba(40,30,10,0.05)]">
                  <img src="https://images.unsplash.com/photo-1588166524941-3bf61a9c41db?auto=format&fit=crop&q=80&w=300" className="w-full h-full object-cover brightness-[0.7] group-hover:scale-105 transition-all duration-300" alt="Indian Main Course" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                  <span className="absolute bottom-3 left-3 text-xs font-serif font-bold text-white">🥘 Indian Mains</span>
                </div>
                <div onClick={() => { setSelectedMobileCategory("pizza"); setActiveTab("menu"); }} className="relative h-24 rounded-2xl overflow-hidden cursor-pointer group shadow-[0_8px_25px_rgba(40,30,10,0.05)]">
                  <img src="https://images.unsplash.com/photo-1513104890138-7c749659a591?auto=format&fit=crop&q=80&w=300" className="w-full h-full object-cover brightness-[0.7] group-hover:scale-105 transition-all duration-300" alt="Pizza" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                  <span className="absolute bottom-3 left-3 text-xs font-serif font-bold text-white">🍕 Pizza & Italian</span>
                </div>
              </div>
            </div>
            ...
            <div className="px-4 space-y-4">
              <h2 className="text-lg font-serif font-bold text-stone-900 uppercase tracking-wide">Guest Ledger Feedback</h2>
          ...
          <div className="animate-fade-in pb-12">
            ...
            <div className="bg-[#FAF9F5]/98 backdrop-blur-xl sticky top-11 z-30 px-4 py-3.5 space-y-3.5 border-b border-stone-200/65 shadow-[0_4px_20px_rgba(40,30,10,0.015)]">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <span className={`w-2 h-2 rounded-full flex items-center justify-center border ${vegOnly ? "border-green-600 bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)] animate-pulse" : "border-stone-400 bg-stone-150"}`}>
                    <span className="w-1 h-1 rounded-full bg-current" style={{ display: vegOnly ? 'block' : 'none' }} />
                  </span>
                  PURE VEG
                </button>
                ...
                >
                  <ThumbsUp className={`w-3.5 h-3.5 ${bestsellerOnly ? "text-[#aa7c11] fill-current" : "text-stone-400"}`} />
                  BESTSELLERS
                </button>
              </div>
                  ...
                  {mobileCategoryChips.map((chip) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="font-sans text-[10px] uppercase tracking-wider">{chip.label.replace(chip.emoji, "").trim()}</span>
                        {isSelected && (
                          <motion.span 
            ...
            <div className="px-4 py-4 space-y-4">
              {filteredMenuList.length === 0 ? (
                <div className="py-16 text-center max-w-xs mx-auto">
                  ...
                  <h4 className="text-sm font-bold text-stone-900 uppercase tracking-wider">No Dishes Found</h4>
                  <p className="text-xs text-stone-500 font-light mt-1.5">Savor another search term or select a broader category!</p>
                </div>
              ) : (
                <div className="space-y-4">
                      ...
                      >
                        ...
                        <div className="flex-grow flex flex-col justify-between min-w-0">
                          <div className="space-y-1">
                            ...
                            <p className="text-[10px] text-stone-500 font-sans leading-relaxed line-clamp-2">{item.description}</p>
                          </div>

                          <div className="flex items-center justify-between pt-2 border-t border-stone-100 mt-1">
                            <span className="text-xs font-mono font-bold text-[#aa7c11]">₹{item.price}</span>
                            ...
                            {qty > 0 ? (
                              <div className="flex items-center gap-2 bg-[#d4af37] rounded-lg p-0.5">
                                ...
                                <span className="text-black font-mono font-black text-xs w-4 text-center">{qty}</span>
                                <button
                                  onClick={() => onAddToCart(item)}
                                  className="w-5.5 h-5.5 rounded bg-stone-900 text-[#d4af37] flex items-center justify-center"
                                >
                              ...
                              >
                                <Plus className="w-2.5 h-2.5 text-[#d4af37]" /> ADD
                              </motion.button>
                            )}
                          </div>
                        </div>
        ...
        search: (
          <div className="px-4 py-6 animate-fade-in pb-12 space-y-4">
            <h2 className="text-lg font-serif font-bold text-stone-900 uppercase tracking-wide flex items-center gap-2">
              <Search className="w-5 h-5 text-[#aa7c11]" /> Direct Instant Search
            </h2>
            ...
            {searchQuery ? (
              <div className="space-y-4">
                <h3 className="text-xs font-mono font-bold text-stone-500 uppercase">Search Results ({filteredMenuList.length})</h3>
                {filteredMenuList.map((item) => {
                  const qty = cart.find((ci) => ci.menuItem.id === item.id)?.quantity || 0;
                  return (
                    <div key={item.id} className="bg-white p-3 rounded-xl border border-stone-200/95 flex gap-3 shadow-[0_4px_15px_rgba(40,30,10,0.02)]">
                      <img src={item.image} alt={item.name} className="w-16 h-16 rounded-lg object-cover bg-stone-105" />
                      <div className="flex-grow min-w-0 flex flex-col justify-between">
                        ...
                        <div className="flex items-center justify-between mt-1">
                          <span className="text-xs font-mono text-[#aa7c11] font-semibold">₹{item.price}</span>
                          {qty > 0 ? (
                            <div className="flex items-center gap-1.5 bg-[#d4af37] rounded p-0.5">
                              <button onClick={() => onRemoveFromCart(item)} className="w-5 h-5 bg-stone-900 text-[#d4af37] rounded flex items-center justify-center">-</button>
                              <span className="text-[10px] text-black font-mono font-bold w-3 text-center">{qty}</span>
                              <button onClick={() => onAddToCart(item)} className="w-5 h-5 bg-stone-900 text-[#d4af37] rounded flex items-center justify-center">+</button>
                            </div>
                          ) : (
                            <button onClick={() => onAddToCart(item)} className="text-[9px] font-mono bg-stone-900 border border-stone-800 text-white px-2 py-0.5 rounded flex items-center gap-0.5 cursor-pointer">+ ADD</button>
                          )}
                        </div>
                      </div>
                    </div>
              ...
              <div className="py-12 text-center space-y-4 max-w-xs mx-auto">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {["Masala Dosa", "Veg Kurkure Momos", "Paneer Butter Masala", "Chilli Paneer", "Cold Coffee", "Chana Masala"].map((tag) => (
                    <button
                      key={tag}
        ...
        cart: (
          <div className="animate-fade-in pb-12">
            <div className="px-4 py-5 border-b border-stone-200/85 flex items-center justify-between bg-white shadow-sm">
              ...
              <span className="text-xs font-mono text-stone-550">({totalItems} Items)</span>
            </div>

            {orderPlaced ? (
              <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
                <CheckCircle2 className="w-16 h-16 text-[#aa7c11] animate-bounce" />
                <h3 className="mt-4 text-lg font-serif font-bold text-stone-900 uppercase tracking-wide">Order Dispatched!</h3>
                <p className="mt-2 text-xs text-stone-550 leading-relaxed font-sans max-w-xs">
                  We have loaded your receipt and items directly to WhatsApp. Click send in chat to start cooking!
                </p>
                <span className="mt-4 text-[10px] font-mono bg-stone-50 border border-stone-150 px-3 py-1 rounded text-stone-505">Connecting kitchen...</span>
              </div>
            ) : cart.length === 0 ? (
              <div className="py-20 text-center space-y-4 max-w-xs mx-auto">
                ...
                <h3 className="text-sm font-bold text-stone-900">Your Plate is Empty</h3>
                <p className="text-xs text-stone-500 leading-relaxed max-w-[200px] mx-auto">Browse through our menu categories and add pure veg dishes to order.</p>
                <button
                  onClick={() => setActiveTab("menu")}
              ...
              <form onSubmit={handleCheckoutSubmit} className="flex flex-col">
                ...
                <div className="px-4 py-4 space-y-3">
                  {cart.map((item) => (
                    <div key={item.menuItem.id} className="bg-white p-3.5 rounded-xl border border-stone-250 flex flex-col gap-2.5 shadow-sm">
                      <div className="flex gap-3">
                        <img src={item.menuItem.image} alt={item.menuItem.name} className="w-12 h-12 rounded-lg object-cover bg-stone-105" />
                        <div className="flex-grow min-w-0">
                          <div className="flex justify-between">
                            <h4 className="text-xs font-bold text-stone-900 truncate font-sans">{item.menuItem.name}</h4>
                            <span className="text-xs font-mono font-bold text-[#aa7c11]">₹{item.menuItem.price * item.quantity}</span>
                          </div>
                          <span className="text-[10px] text-stone-400 font-mono">₹{item.menuItem.price} each</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-stone-100">
                        ...
                        <div className="flex items-center gap-1.5 bg-stone-50 p-0.5 rounded border border-stone-200">
                          <button type="button" onClick={() => onRemoveFromCart(item.menuItem)} className="w-5 h-5 text-stone-500 hover:text-stone-900">-</button>
                          <span className="text-[11px] font-mono text-stone-900 font-bold w-4 text-center">{item.quantity}</span>
                          <button type="button" onClick={() => onAddToCart(item.menuItem)} className="w-5 h-5 text-stone-500 hover:text-stone-900">+</button>
                        </div>
                      </div>
                    </div>
                  ))}
                  ...
                  {showOtpMobile ? (
                    <div className="space-y-4 font-sans text-left">
                      <div className="flex items-center gap-2 text-[#aa7c11]">
                        ...
                        <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-[#aa7c11]">OTP Security Gate</span>
                      </div>

                      <p className="text-stone-500 text-xs leading-relaxed font-sans">
                        To reserve your kitchen service and complete checkout, a simulated 4-digit code has been dispatched via cellular gateway to: <strong className="text-stone-900">{phoneNumber || "+91 11-4560-4560"}</strong>.
                      </p>

                      <div className="space-y-1.5">
                        <label className="text-[9px] text-stone-550 font-mono block">ENTER 4-DIGIT VERIFICATION CODE</label>
                        <input
                          type="text"
                          maxLength={4}
                          required
                          placeholder="••••"
                          value={otpCodeMobile}
                          onChange={(e) => setOtpCodeMobile(e.target.value.replace(/\D/g, ""))}
                        ...
                        />
                        {otpErrorMobile && (
                          <p className="text-[10px] text-red-650 font-sans mt-1">{otpErrorMobile}</p>
                        )}
                      </div>
                      ...
                      <div className="p-2.5 bg-amber-50 border border-amber-200 rounded-xl text-[9px] text-stone-750 font-mono flex items-center justify-between">
                        ...
                        <strong className="text-xs font-bold text-[#aa7c11] tracking-wide font-mono">{generatedOtpMobile}</strong>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-1 text-xs">
                        ...
                        {otpCountdownMobile > 0 ? (
                          <span className="text-stone-400 font-mono text-[10px]">Resend in {otpCountdownMobile}s</span>
                        ) : (
                          <button
                            type="button"
                    ...
                    <>
                      ...
                      <div className="space-y-3">
                        <div>
                          <label className="text-[9px] text-stone-550 font-mono block mb-1">YOUR NAME *</label>
                          <input
                            type="text"
                            required
                        ...
                        <div>
                          <label className="text-[9px] text-stone-550 font-mono block mb-1">PHONE NUMBER (OPTIONAL)</label>
                          <input
                            type="tel"
                        ...
                        {orderType === "delivery" && (
                          <div>
                            <label className="text-[9px] text-stone-550 font-mono block mb-1">DELIVERY ADDRESS *</label>
                            <textarea
                              required
                ...
                <div className="bg-white border-t border-stone-205 p-4 space-y-3 pb-8 shadow-[0_-8px_30px_rgba(40,30,10,0.02)]">
                  <div className="space-y-1.5 text-xs">
                    ...
                    {packagingCharge > 0 && (
                      <div className="flex justify-between text-stone-500">
                        ...
                        <span className="font-mono font-medium">₹{packagingCharge}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-stone-900 text-base font-bold pt-2 border-t border-stone-105">
                      <span className="font-serif italic font-medium">Total Price</span>
                      <span className="font-mono text-[#aa7c11]">₹{grandTotal}</span>
                    </div>
                  </div>
                  ...
                  >
                    <Send className={`w-4 h-4 text-white fill-current ${isSubmittingMobile ? "animate-bounce" : ""}`} /> 
                    {isSubmittingMobile 
                      ? "RESERVING DISHES..." 
                      : showOtpMobile 
                      ? "VERIFY OTP & CONFIRM" 
          ...
          <div className="px-4 py-6 animate-fade-in pb-12 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <span className="text-xs font-bold text-stone-900">Call Kitchen</span>
                <span className="text-[10px] text-stone-500">+91-11-45604560</span>
              </a>
              ...
              >
                ...
                <span className="text-xs font-bold text-stone-900">WhatsApp Direct</span>
                <span className="text-[10px] text-stone-500">Live Delivery</span>
              </a>
            </div>
            ...
            <div className="bg-white border border-stone-200 p-5 rounded-3xl space-y-4 shadow-sm">
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-stone-555 mt-1">Open 7 days a week</p>
                  <p className="text-[10px] font-mono text-[#aa7c11] mt-0.5 font-bold">11:00 AM - 11:30 PM DAILY</p>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-stone-900 uppercase tracking-wide">Restaurant Address</h4>
            ...
            <div className="space-y-3">
              <h3 className="text-xs font-mono font-bold text-stone-500 uppercase">Find on Google Maps</h3>
            ...
            <div className="flex justify-center items-center gap-5 pt-4">
              <a href="https://facebook.com" className="w-10 h-10 rounded-full bg-white border border-stone-200 flex items-center justify-center text-stone-500 hover:text-[#aa7c11] shadow-sm"><Facebook className="w-4 h-4" /></a>
              <a href="https://instagram.com" className="w-10 h-10 rounded-full bg-white border border-stone-200 flex items-center justify-center text-stone-500 hover:text-[#aa7c11] shadow-sm"><Instagram className="w-4 h-4" /></a>
              <a href="https://twitter.com" className="w-10 h-10 rounded-full bg-white border border-stone-200 flex items-center justify-center text-stone-500 hover:text-[#aa7c11] shadow-sm"><Twitter className="w-4 h-4" /></a>
            </div>
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-[#aa7c11]">
                  ...
                  <h3 className="font-serif font-bold text-stone-900 uppercase tracking-wide text-sm">WhatsApp Fast Inquiry</h3>
                </div>
                <button onClick={() => setShowQuickOrder(false)} className="w-8 h-8 rounded-full bg-stone-100 flex items-center justify-center text-stone-500 hover:bg-stone-250 cursor-pointer">
                  <X className="w-4 h-4" />
                </button>
              </div>

              <p className="text-xs text-stone-500 leading-relaxed font-light">
                Directly chat with Sagar Ratna kitchen chefs to inquire about special requests, catering pricing, or allergen parameters.
              </p>
              ...
              <form onSubmit={handleQuickWhatsAppQuery} className="space-y-4">
                ...
                >
                  <MessageSquare className="w-3.5 h-3.5 fill-current" /> Open WhatsApp Chat
                </button>
              </form>
            </motion.div>
          </>
        ...
        {totalItems > 0 && activeTab !== "cart" && (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] uppercase tracking-wider font-extrabold pr-1">Total Plates in Basket</span>
            </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-stone-200/80 z-50 py-2.5 px-4 flex justify-between items-center select-none shadow-[0_-8px_30px_rgba(0,0,0,0.04)]">
        {[
          ...
          { tab: "cart" as const, label: "Cart", icon: ShoppingBag, badge: totalItems },
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center w-16 relative py-1 focus:outline-none transition-colors duration-250 cursor-pointer ${
                isActive ? "text-[#aa7c11]" : "text-stone-400 hover:text-stone-700"
              }`}
            >
              ...
              <span className="text-[9px] font-medium mt-1 leading-none tracking-wide font-sans">{label}</span>
...
function animateContainer(isLoading: boolean, activeTab: string, views: {[key: string]: React.ReactNode}) {
